...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            rows:[],
            columns:[],
            // cells definitions ---- custom modification
            cells:[],
        ...
        */
        ...
        obj.createCell = function(i, j, value) {
            ...
            let objCell = null;
            let objValuesBeforeModif = null;
            if (obj.options.cells.length>0) {
                //cell modifier appiered
                objCell = obj.options.cells.find(objCell => (objCell.row === +j && objCell.col === +i));
                if(objCell){
                    if(objCell.type){
                        objValuesBeforeModif = {"type":obj.options.columns[i].type,"source":obj.options.columns[i].source};
                        obj.options.columns[i].type = objCell.type;
                        if(obj.options.columns[i].type==='dropdown' && objCell.source){
                            obj.options.columns[i].source = objCell.source;
                        }
                    }
                }
            }
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    td.innerText = obj.getDropDownValue(i, j, value);
            ...
            // ---- custom modification - start
            if(objCell){
                obj.options.columns[i].type = objValuesBeforeModif.type;
                obj.options.columns[i].source = objValuesBeforeModif.source;
            }
            // ---- custom modification - end
        ...
        obj.openEditor = function(cell, empty, e) {
            ...
            if (obj.options.cells.length>0) {
                ...
                if(objCell){
                    if(objCell.type){
                        objValuesBeforeModif = {"type":obj.options.columns[x].type,"source":obj.options.columns[x].source};
                        obj.options.columns[x].type = objCell.type;
                        if(obj.options.columns[x].type==='dropdown' && objCell.source){
                            obj.options.columns[x].source = objCell.source;
                        }
                    }
                }
            }
            ...
            // ---- custom modification - start
            if(objCell){
                obj.options.columns[x].type = objValuesBeforeModif.type;
                obj.options.columns[x].source = objValuesBeforeModif.source;
            }
            // ---- custom modification - end
        }
        ...
        obj.updateCell = function(x, y, value, force) {
            ...
            if (obj.records[y][x].classList.contains('readonly') == true && ! force) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    // Native functions
                    if (strCellType == 'checkbox' || strCellType == 'radio') {
                        // Unchecked all options
                        if (strCellType == 'radio') {
                            for (var j = 0; j < obj.options.data.length; j++) {
                        ...
                        // Update data and cell
                        obj.options.data[y][x] = value;
                        obj.records[y][x].innerText = obj.getDropDownValue(x, y, value);
                    } else if (strCellType == 'calendar') {
                        // Try formatted date
                        var formatted = null;
                        if (! validDate(value)) {
         ...
         */
        obj.getDropDownValue = function(column, row, key) {
            var value = [];
            ...
            if (obj.options.cells.length>0) {
                //cell modifier appiered
                objCell = obj.options.cells.find(objCell => (objCell.row === +row && objCell.col === +column));
                if(objCell){
                    if(objCell.type==='dropdown' && objCell.source){
                        alternativeSource = objCell.source;
                    }
                }
            }
            // ---- custom modification - end
            if (obj.options.columns[column] && (obj.options.columns[column].source || alternativeSource)) {
                // Create array from source
                var combo = [];
                var source = alternativeSource?alternativeSource:obj.options.columns[column].source;
